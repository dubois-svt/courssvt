# Activité : Cahier des charges

!!! note "Compétences"
    - Trouver et utiliser des informations


Vous êtes responsable du projet de rénovation d’une salle de sciences. Vous allez devoir compléter votre cahier des charges d'après les textes de recommandations et proposer un croquis d'une salle de classe.

!!! warning "Consignes"
    1. Compléter le cahier des charges en retrouvant les éléments manquants.
    2. Produire un diagramme des cas d'utilisation de la salle de classe. 
    3. Réaliser un croquis rapide de la salle de classe.

??? bug "Critères de réussite"
    - Avoir complété le tableau du document 4
    - Avoir proposé un croquis de la salle de classe
    - Avoir mis une légende et un titre au croquis


<div markdown style="break-inside: avoid;">


**Document 1 : Recommandations pour une salle de classe**  

- Rangement : pour le matériel pédagogique devant rester dans la salle et pour le matériel et les effets personnels des enseignants et des élèves.  
- Flexibilité de configuration pour un travail individuel et/ou collectif, avec suffisamment de place pour une classe complète de 24 élèves.  

</div>
<div markdown style="break-inside: avoid;">

**Document 2 : Des normes dans la construction des bâtiments publiques**  

Pour la sécurité des usagers, une salle de classe et plus particulièrement une salle de sciences, doit respecter quelques normes.

- Il doit y avoir deux sorties d'urgence clairement indiquées dans la salle de classe. Ces sorties doivent être facilement accessibles et dégagées en tout temps.
- Un extincteur doit être présent en permanence et aisément accessible.
- Dans les salles de sciences, un point d'eau doit être installé pour permettre un nettoyage du matériel et un rinçage en cas de projection de produits.
- Un équipement pour les personnes à mobilité réduite doit être installé, dans une de classe, il s'agira d'une table avec assez d'espace autour pour qu'un fauteuil roulant circule.

</div>
<div markdown style="break-inside: avoid;">

**Document 3 Disposition des fenêtres**

Le Soleil est visible à l'est le matin et à l'ouest en fin d'après midi et il est plutôt dans la partie Sud du ciel.
Donc en fonction d'où l'on dispose les fenêtres d'un bâtiment, la conséquence ne sera pas la même.

- Au nord : peu de lumière et le soleil chauffera peu la pièce.
- A l'est : beaucoup de Soleil le matin, peu l'après-midi
- A l'ouest : beaucoup de soleil l'après-midi, peu le matin
- Au sud : un éclairement assez constant sur la journée.

</div>
<div markdown style="break-inside: avoid;">


**Document 4 Cahier des charges de la salle de classe**



<table><thead>
  <tr>
    <th style="background-color:LightGray">Type de contraintes</th>
    <th style="background-color:LightGray">Contraintes</th>
    <th style="background-color:LightGray">Solutions</th>
  </tr></thead>
<tbody>
  <tr>
    <td rowspan="4"></td>
    <td>Lumineux en permanence</td>
    <td>4 appliques ou suspension&nbsp;&nbsp;lumineuse</td>
  </tr>
  <tr>
    <td>travail des élèves</td>
    <td></td>
  </tr>
  <tr>
    <td>travail de l'enseignant</td>
    <td>1 bureau, 1 ordinateur, 1 tableau et 1 chaise</td>
  </tr>
  <tr>
    <td>Température supérieure à 18°C en permanence</td>
    <td></td>
  </tr>
  <tr>
    <td></td>
    <td>possibilité d'allumer la lumière et le chauffage le moins souvent possible.</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td>Capacité de la salle à accueillir un élève en fauteuil roulant</td>
    <td>place assez large pour un fauteuil roulant, près de l'entrée</td>
  </tr>
  <tr>
    <td>accès à de l'eau pour le nettoyage du matériel</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="3"></td>
    <td rowspan="2">Respect des normes</td>
    <td></td>
  </tr>
  <tr>
    <td></td>
  </tr>
  <tr>
    <td>Rangement fermé pour le matériel</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">demande du conseil général de colorer la salle</td>
    <td>Au moins 1 mur  avec une couleur claire </td>
  </tr>
  <tr>
    <td>sol avec une couleur beige ou brune</td>
  </tr>
</tbody></table>





</div>


??? note-prof "correction"


    <table><thead>
      <tr>
        <th style="background-color:LightGray">Type de contraintes</th>
        <th style="background-color:LightGray">Contraintes</th>
        <th style="background-color:LightGray">Solutions</th>
      </tr></thead>
    <tbody>
      <tr>
        <td rowspan="4">Contraintes de fonctionnement</td>
        <td>Lumineux en permanence</td>
        <td>4 appliques ou suspension&nbsp;&nbsp;lumineuse</td>
      </tr>
      <tr>
        <td>travail des élèves</td>
        <td>24 places (tables et chaises)</td>
      </tr>
      <tr>
        <td>travail de l'enseignant</td>
        <td>1 bureau, 1 ordinateur, 1 tableau et 1 chaise</td>
      </tr>
      <tr>
        <td>Température supérieure à 18°C en permanence</td>
        <td>radiateurs</td>
      </tr>
      <tr>
        <td>économique et environnementales</td>
        <td>possibilité d'allumer la lumière et le chauffage le moins souvent possible.</td>
        <td>fenêtres côté sud</td>
      </tr>
      <tr>
        <td rowspan="2">Ergonomique</td>
        <td>Capacité de la salle à accueillir un élève en fauteuil roulant</td>
        <td>place assez large pour un fauteuil roulant</td>
      </tr>
      <tr>
        <td>accès à de l'eau pour le nettoyage du matériel</td>
        <td>1 robinet </td>
      </tr>
      <tr>
        <td rowspan="3">Sécurité</td>
        <td rowspan="2">Respect des normes de sécurité</td>
        <td>extincteur</td>
      </tr>
      <tr>
        <td>2 portes dans la salle</td>
      </tr>
      <tr>
        <td>Rangement fermé pour le matériel</td>
        <td>armoire avec porte</td>
      </tr>
      <tr>
        <td rowspan="2">Esthétique</td>
        <td rowspan="2">demande du conseil général de colorer la salle</td>
        <td>Au moins 1 mur  avec une couleur claire </td>
      </tr>
      <tr>
        <td>sol avec une couleur beige ou brune</td>
      </tr>
    </tbody></table>
